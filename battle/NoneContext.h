...
class NoneContext : public GameObjectContext
{
private:
	ability::abilities_t abilities;

	mutable Effects effects;
	mutable ActiveEffects *active_effects;
	mutable PassiveAbilities *passive_abilities;

public:
	static const GameObject::Id NONE_ID;

	NoneContext();
	NoneContext(ability::abilities_t abilities);
	~NoneContext();
	...
	void check_quest(const quests::Condition *consition, uint32_t amount = 1) override;
